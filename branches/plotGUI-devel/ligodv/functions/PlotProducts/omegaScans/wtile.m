...
if minimumQ > maximumQ,
  msg = 'minimum Q exceeds maximum Q';
      mymsg = sprintf(msg);
      mb = msgbox(mymsg, 'Input Error', 'error');
      waitfor(mb);
...
if minimumFrequency > maximumFrequency,
  msg = 'minimum frequency exceeds maximum frequency';
      mymsg = sprintf(msg);
      mb = msgbox(mymsg, 'Input Error', 'error');
      waitfor(mb);
...
if minimumQ < minimumAllowableQ, 
  msg = 'minimum Q less than %3.2f\n';
      mymsg = sprintf(msg, minimumAllowableQ);
      mb = msgbox(mymsg, 'Input Error', 'error');
      waitfor(mb);
...
if  maximumMismatch > maximumAllowableMismatch,
    msg = 'maximum mismatch exceeds %.2f\n';
      mymsg = sprintf(msg, maximumAllowableMismatch);
      mb = msgbox(mymsg, 'Input Error', 'error');
      waitfor(mb);
...
for plane = 1 : numberOfPlanes,
  ...
  if tiling.minimumFrequency <= minimumAllowableFrequency,
    minimumFrequency = minimumAllowableFrequency;
  end

  % use plane specific maximum allowable frequency if requested
  if tiling.maximumFrequency >= maximumAllowableFrequency,
    maximumFrequency = maximumAllowableFrequency;
  end
...
if (2 * tiling.transientDuration) >= tiling.duration,
    msg = 'duration of filter transients equals or exceeds data duration';
      mymsg = sprintf(msg);
      mb = msgbox(mymsg, 'Input Error', 'error');
      waitfor(mb);
