function handles = dv_weventgram(events, tiling, startTime, referenceTime, ...
                              timeRange, frequencyRange, ...
                              durationInflation, bandwidthInflation, ...
                              normalizedEnergyRange, plotAmplitudes)
...
%     plotAmplitudes          scalar, will plot amplitude instead of SNR if
%                             not empty or zero
%
...
% $Id: dv_weventgram.m,v 1.4 2012/06/15 13:44:53 michalw Exp $
...
if (nargin < 10) || isempty(plotAmplitudes),
  plotAmplitudes = [];
end
...
for channelNumber = 1 : numberOfChannels,
  if isfield(events{channelNumber},'channelName'),
...
handles = gobjects(numberOfChannels, 1);
...
for channelNumber = 1 : numberOfChannels,
  ...
  % Replace energies with SNR
  if plotAmplitudes
    normalizedEnergies = events{channelNumber}.amplitude;
  else
    normalizedEnergies = sqrt(2*normalizedEnergies);
  end
